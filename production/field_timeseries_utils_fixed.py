...
def get_table_bounds_geometry(gdf):
...
def create_image_collection(geom, start_year=2024, start_month=11, start_day=1,
    ...
    return ee.ImageCollection.fromImages(image_list), geom


def process_field_combination(gdf, image_collection, collection_bounds, campo_value, lote_value, output_dir):
    """Process a single campo-lote combination with intersection checking"""
    ...
    intersects = check_geometry_intersection(dissolved_geometry, collection_bounds)
    if not intersects:
...
def generate_field_timeseries(schema, output_dir, database_uri=None, table_name=None,
                             ...
                             end_year=2025, end_month=4, end_day=21, cloud_cover=20):
    """
    Main function to generate time series for all fields using optimized approach
    ...
    for table in table_list:
        ...
        # Create a bounding box geometry that encompasses all fields in the table (OPTIMIZED APPROACH)
        table_bounds_geom = get_table_bounds_geometry(filtered_gdf)
        
        # Create image collection using table bounds
        print("🚀 Creating collection once for entire table (optimized approach)...")
        image_collection, collection_bounds = create_image_collection(
        ...
        for _, row in combinations.iterrows():
            success = process_field_combination(
                filtered_gdf, image_collection, collection_bounds,
        ...
        if skipped > 0:
            print(f"⚠️  Skipped (no intersection or errors): {skipped}")
        print(f"🎯 Collection created only once for entire table (optimized!)")
    
    print(f"\n🎉 All processing complete! Check output directory: {output_dir}")
    print(f"🚀 Used optimized approach: Collection created once per table for maximum efficiency!")
